...
# Bug Warning(s):
#   - Best ran and compiled using Python Idle 
#   - Expected results tend to have overlapping rules
#   - Sometimes crashes when you declare 3rd rule
...
class CFG:
    ...
    _V = []                                         # Variables - Non Terminal Symbol
    _SIGMA = []                                     # Alphabet - Terminal Symbols
    _S = None                                       # Start Symbol
    _P = []                                         # Productions
    _V_set = '[A-Z](_[0-9]*)?(,[A-Z](_[0-9]*)?)*'   # Accepted Variables - Non Terminal Symbols - RegEx
    _SIGMA_set = '.*(,.*)*'                         # Accepted Alphabet - Terminal Symbols - RegExp
    _S_set = '[A-Z](_[0-9]*)?'                      # Accepted Start Symbol - RegExp
    _P_set = '''([A-Z](_[0-9]*)?->.*(|.*)*(,
             [A-Z](_[0-9]*)?->.*(|.*)*)*)'''        # Accepted Productions - RegExp
